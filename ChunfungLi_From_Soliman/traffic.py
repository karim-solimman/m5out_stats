...
triBA27_m5_nodes = ['system.ruby.network.routers' + val for val in triBA27_nodes]

traffic = ['generated_packets', 'forwarded_packets', 'received_packets']
vnets = ['vnet_0_counter', 'vnet_1_counter', 'vnet_2_counter']
vnet_0_counter, vnet_1_counter, vnet_2_counter = 0, 0, 0
...
if __name__ == "__main__":
    ...
    work_book = openpyxl.load_workbook(
        "/home/soliman/m5out_stats/ChunfungLi_From_Soliman/m5out_stats_traffic.xlsx")
    ...
    print(os. getcwd())
    m5_file = open("/home/soliman/m5out_stats/ChunfungLi_From_Soliman/stats.txt")
    ...
    for line in m5_file:
        ...
        for node in triBA27_m5_nodes:
            ...
            for vnet in vnets:
                line_to_search = node + '.' + vnet
                splitted_line = line.split()
                if line_to_search in splitted_line:
                    print(splitted_line, len(splitted_line))
                    data.append(float(splitted_line[1]))
                    if "vnet_0_counter" in line_to_search:
                        vnet_0_counter += float(splitted_line[1])
                    elif "vnet_1_counter" in line_to_search:
                        vnet_1_counter += float(splitted_line[1])
                    elif "vnet_2_counter" in line_to_search:
                        vnet_2_counter += float(splitted_line[1])
    data.insert(7,vnet_0_counter)
    data.insert(8,vnet_1_counter)
    data.insert(9,vnet_2_counter)
